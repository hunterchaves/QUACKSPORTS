...
class VenueRepository(private val db: FirebaseFirestore = FirestoreProvider.db) {
    ...
    fun allCourts(): Flow<List<Court>> = callbackFlow {
        val reg = db.collectionGroup("courts").addSnapshotListener { snap, err ->
            if (err != null) { trySend(emptyList()); close(); return@addSnapshotListener }
            trySend(snap?.documents?.mapNotNull {
                val court = it.toObject(Court::class.java)?.copy(id = it.id)
                val venueId = it.reference.parent.parent?.id ?: ""
                court?.copy(venueId = venueId)
            } ?: emptyList())
        }
        awaitClose { reg.remove() }
    }
